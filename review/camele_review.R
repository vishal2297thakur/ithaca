# Review camele data ----
source('source/partition_evap.R')
source("source/cdo_functions.R")
source("source/geo_functions.R")

library(pRecipe)

## geospatial 
library(raster)
library(ncdf4)
library(sp)
library(sf)
library(stars)

## stats
library("openair")

## Load data ----
fnames_nc <- list.files(path = PATH_EVAP_SIM, pattern = ".nc$", 
                        full.names = T)

fnames_product <- fnames_nc[grep("camele", fnames_nc)]

fname <- fnames_product[grep("yearly", fnames_product)]

## 1. Check name: Data product, variables, unit, scale, beginning of time, end of time, spatial resolution, temporal resolution ----
fname

## 2. Check unit conversion/scaling (nc history)----
# multipliers "-mulc"
# muldpm
cmd_ncdump <- paste0("ncdump -h ", fname)
system(cmd_ncdump)

# variable name
cmd_cdo_variable_name <- paste0("cdo vardes ", fname)
system(cmd_cdo_variable_name)

# summary
cdo_info_fnc(fname)

## 3. plot map to see if it looks okay ----
data <- brick(fname) 
pRecipe::plot_map(data[[12]])

## 4. Check projection ----
# projection = lonlat
# lon: from east to west and -180 to 180 (1440)
# lat:from north to south and 90 to -90 (720)
crs(data)
extent(data)

# OG grid location for comparison
cmd_cdo_griddes <- paste0("cdo -griddes ", fname)
system(cmd_cdo_griddes)

grid_location <- "/mnt/shared/data_projects/ithaca/misc/grid_ithaca"
cmd_cat_grid <- paste0("cat ", grid_location)
system(cmd_cat_grid)

## 5. Check time units ----
# daily: hours should always be 00:00:00
# monthly: day should always be 01
# yearly: month and day should be 01-01
cdo_sinfo_fnc(fname)
data@z$Date

## 6. Check data type: Data type F32z ----
# scroll to top of output
cdo_sinfo_fnc(fname)


## 7. Compare values to at least one other publication ----
cdo_timmean_fnc(inputfile_name = fname, outputfile_name = "~/Review/camele_review_time_mean.nc")
cdo_info_fnc(inputfile_name = "~/Review/camele_review_time_mean.nc")

# Comparison to product paper (Li et al. 2024, ESSD, accepted paper) reporting multi-year mean 2000-2017
period_start <- as.Date("2000-01-01") 
period_end <- as.Date("2017-01-01") 

start_time <- which(data@z$Date == period_start)
end_time <- which(data@z$Date == period_end)

cdo_seltimestep_fnc(inputfile_name = fname, outputfile_name = "~/Review/camele_review_sel.nc", start_time = start_time, end_time = end_time)

cdo_timmean_fnc(inputfile_name =  "~/Review/camele_review_sel.nc", outputfile_name = "~/Review/camele_timmean_mean.nc")
cdo_info_fnc(inputfile_name = "~/Review/camele_timmean_mean.nc")

means <- raster("~/Review/camele_timmean_mean.nc")
means_dt <- brick_to_dt(means)

means_dt[, value_daily := value/365]
means_dt[, value_daily_brk := cut(value_daily, seq(0, 5, 0.5))]


ggplot(means_dt)+
  geom_tile(aes(x = x, y = y, fill = value_daily_brk, col = value_daily_brk))+
  scale_fill_manual(values = c("#0000ff", "#3661ff", "#38acff", "#00ffff","#91ffb4", "#d2ff69", "#ffff00", "#ffb700", "#ff6f00", "#ff0000"))+
  scale_color_manual(values = c("#0000ff", "#3661ff", "#38acff", "#00ffff","#91ffb4", "#d2ff69", "#ffff00", "#ffb700", "#ff6f00", "#ff0000"))+
  ggtitle(label = "camele")+
  theme_bw()
# Change according to need
# Comparison to product paper (Li et al. 2023, ESSD) reporting daily trend map 1980-2022
period_start <- as.Date("1980-01-01") 
period_end <- as.Date("2022-01-01") 

start_time <- which(data@z$Date == period_start)
end_time <- which(data@z$Date == period_end)

cdo_seltimestep_fnc(inputfile_name = fname, outputfile_name = "~/Review/camele_review_sel.nc", start_time = start_time, end_time = end_time)

cdo_timmean_fnc(inputfile_name =  "~/Review/camele_review_sel.nc", outputfile_name = "~/Review/camele_timmean_mean.nc")
cdo_info_fnc(inputfile_name = "~/Review/camele_timmean_mean.nc")
cmd_cdo_trend <- paste0("cdo trend ~/Review/camele_review_sel.nc ~/Review/camele_review_sel_trend_a.nc ~/Review/camele_review_sel_trend_b.nc")
system(cmd_cdo_trend)

cmd_cdo_div <- paste0("cdo div ~/Review/camele_review_sel_trend_b.nc ~/Review/camele_timmean_mean.nc ~/Review/camele_trend_percent.nc")
system(cmd_cdo_div)
trends <- raster("~/Review/camele_trend_percent.nc")
plot(trends*100)
trends_dt <- brick_to_dt(trends)

trends_dt[, slope_percent_brk := cut(value*100,c(-8, seq(-2.0,1.5,0.5),7))]

ggplot(trends_dt)+
  geom_tile(aes(x = x, y = y, fill = slope_percent_brk))+
  scale_fill_manual(values = c("#228292", "#5c9e9c", "#91bca8", "#c7deb4","#ffffc1", "#e8cf8f", "#cfa163", "#b5773e", "#9d541f"))+
  ggtitle(label = "camele")+
  theme_bw()

# Comparison to review paper (yang et al. 2023, Nature Reviews Earth & Environment) reporting trend for (i) 1982-2020, (ii) 1982-2011 (SI), (iii) 2001-2020 (SI)
# trend i
period_start <- as.Date("1982-01-01") 
period_end <- as.Date("2020-01-01") 

start_time <- which(data@z$Date == period_start)
end_time <- which(data@z$Date == period_end)

cdo_seltimestep_fnc(inputfile_name = fname, outputfile_name = "~/Review/camele_review_sel_i.nc", start_time = start_time, end_time = end_time)
cdo_fldmean_fnc(inputfile_name = "~/Review/camele_review_sel_i.nc", outputfile_name = "~/Review/camele_review_sel_fldmean.nc")

cmd_cdo_trend <- paste0("cdo trend ~/Review/camele_review_sel_fldmean.nc ~/Review/camele_review_sel_fldmean_trend_a.nc ~/Review/camele_review_sel_fldmean_trend_b.nc")
system(cmd_cdo_trend)
cdo_info_fnc(inputfile_name = "~/Review/camele_review_sel_fldmean_trend_b.nc")

fld_means <- cdo_info_to_text_fnc(inputfile_name = "~/Review/camele_review_sel_fldmean.nc", outputfile_name = "~/Review/camele_1982_2020.txt")
fld_means[, year := as.numeric(format(date, "%Y"))]
slope <- theil_sen_regression(value ~ year, data = fld_means)$coefficients[2]
lm(value ~ year, data = fld_means)
mean_fld <- mean(fld_means$value, na.rm = T)
plot(fld_means$date, fld_means$value-mean_fld, type = "b", ylim = c(-30, 30))

# trend ii
period_start <- as.Date("1982-01-01") 
period_end <- as.Date("2011-01-01") 

start_time <- which(data@z$Date == period_start)
end_time <- which(data@z$Date == period_end)

cdo_seltimestep_fnc(inputfile_name = fname, outputfile_name = "~/Review/camele_review_sel_ii.nc", start_time = start_time, end_time = end_time)
cdo_fldmean_fnc(inputfile_name = "~/Review/camele_review_sel_ii.nc", outputfile_name = "~/Review/camele_review_sel_fldmean.nc")
cmd_cdo_trend <- paste0("cdo trend ~/Review/camele_review_sel_fldmean.nc ~/Review/camele_review_sel_fldmean_trend_a.nc ~/Review/camele_review_sel_fldmean_trend_b.nc")
system(cmd_cdo_trend)
cdo_info_fnc(inputfile_name = "~/Review/camele_review_sel_fldmean_trend_b.nc")
fld_means <- cdo_info_to_text_fnc(inputfile_name = "~/Review/camele_review_sel_fldmean.nc", outputfile_name = "~/Review/camele_1982_2011.txt")
fld_means[, year := as.numeric(format(date, "%Y"))]
slope <- theil_sen_regression(value ~ date, data = fld_means)$coefficients[2]
lm(value ~ date, data = fld_means)

# trend iii
period_start <- as.Date("2001-01-01") 
period_end <- as.Date("2020-01-01") 

start_time <- which(data@z$Date == period_start)
end_time <- which(data@z$Date == period_end)

cdo_seltimestep_fnc(inputfile_name = fname, outputfile_name = "~/Review/camele_review_sel_iii.nc", start_time = start_time, end_time = end_time)
cdo_fldmean_fnc(inputfile_name = "~/Review/camele_review_sel_ii.nc", outputfile_name = "~/Review/camele_review_sel_fldmean.nc")
cmd_cdo_trend <- paste0("cdo trend ~/Review/camele_review_sel_fldmean.nc ~/Review/camele_review_sel_fldmean_trend_a.nc ~/Review/camele_review_sel_fldmean_trend_b.nc")
system(cmd_cdo_trend)
cdo_info_fnc(inputfile_name = "~/Review/camele_review_sel_fldmean_trend_b.nc")
fld_means <- cdo_info_to_text_fnc(inputfile_name = "~/Review/camele_review_sel_fldmean.nc", outputfile_name = "~/Review/camele_2001_2020.txt")
fld_means[, year := as.numeric(format(date, "%Y"))]
slope <- theil_sen_regression(value ~ year, data = fld_means)$coefficients[2]
lm(value ~ year, data = fld_means)


