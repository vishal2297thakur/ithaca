...
prec_annual <- annual_prec
setDT(prec_annual)
prec_annual[, year := 2000:2019]
prec_annual <- melt(prec_annual, id.vars = 'year')

mean(dummy_annual[11:20]) -  mean(dummy_annual[1:10])


sample_size <- 10000
mc_sample <- replicate(sample_size, sample(dummy_annual[1:20], 20, replace = TRUE))
...
## GPM
dummy_raster <- prec_2000_2019[[10]]
dummy <- data.table(time = as.Date(sub('.', '', names(dummy_raster)), format = "%Y.%m.%d"))
dummy[, mean_monthly := cellStats(dummy_raster, "mean")]
dummy[, sum_annual := mean(mean_monthly), year(time)]
dummy_annual <- dummy[, unique(sum_annual)]
obs_mean_diff <-  mean(dummy_annual[11:20]) -  mean(dummy_annual[2:10])

sample_size <- 10000
mc_sample <- replicate(sample_size, sample(dummy_annual[2:20], 20, replace = TRUE))
mc_sample_means <- data.table(annual_diff = apply(mc_sample[1:10, ], 2, mean) - 
                                apply(mc_sample[11:20, ], 2, mean))
