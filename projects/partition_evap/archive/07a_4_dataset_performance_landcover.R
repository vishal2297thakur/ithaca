...
datasets_vol <- data.table(read.csv(paste0(PATH_SAVE_PARTITION_EVAP_TABLES, 
                                           "partition_land_cover_datasets_global.csv")))[, 2:12]
...
landcover_means <- datasets_vol[, .(evap_mean = mean(evap)), landcover]
datasets_mean_ratio <- datasets_vol_matrix / landcover_means$evap_mean
...
#ggsave(paste0(PATH_SAVE_PARTITION_EVAP_FIGURES, "dataset_performance_landcover_ver1.png"), width = 15, height = 7)
...
ggsave(paste0(PATH_SAVE_PARTITION_EVAP_FIGURES,
              "dataset_performance_landcover_ver2.png"), width = 8, height = 12)
