...
### Elevation ----
elev <- evap_stats_masks[,.(stats_volume = sum(evap_volume)),.(ratio_std_quantile_brk, elev_class)]
elev <- elev[complete.cases(elev)]
elev[, land_area:= sum(stats_volume), .( elev_class)]
elev[, land_fraction:= stats_volume/land_area]
...
evap_stats <- evap_stats_masks[,.(sqr_group_area = sum(evap_volume)),.(ratio_std_quantile_brk, evap_quant)]
...
ggsave(paste0(PATH_SAVE_PARTITION_EVAP_FIGURES, "supplement/fig4_SI_SQR_aridity_indices_elev_evap_volume.png"), 
       width = 8, height = 12)
