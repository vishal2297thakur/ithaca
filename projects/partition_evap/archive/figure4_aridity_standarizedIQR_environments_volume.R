...
evap_stats[, summary(ratio_std_quantile)]
...
evap_mean <- evap_vol_mean[, .(evap_volume = mean(evap_volume), area = mean(area)), .(lat, lon)]
evap_stats_masks <- evap_mean[evap_stats_masks, on = .(lon, lat)]

### Global ----
global <- evap_stats_masks[,.(stats_volume = sum(evap_volume), stats_area = sum(area)),.(ratio_std_quantile_brk)]
global[, fraction := stats_volume/sum(stats_volume)]
global[, area_fraction := stats_area/sum(stats_area)]
global
### Land cover ----
landuse <- evap_stats_masks[,.(stats_volume = sum(evap_volume), stats_area = sum(area)),.(ratio_std_quantile_brk, land_cover_short_class)]
landuse <- landuse[complete.cases(landuse)]
landuse[, sum_volume:= sum(stats_volume), .( land_cover_short_class)]
landuse[, sum_area:= sum(stats_area), .( land_cover_short_class)]
landuse[, volume_fraction:= stats_volume/sum_volume]
landuse[, area_fraction:= stats_area/sum_area]

data_high <- landuse[ratio_std_quantile_brk == "dry > wet             ",
                     .(fraction = sum(volume_fraction)), land_cover_short_class]
...
fig_a_spi_landuse <- ggplot(landuse[land_cover_short_class != "Other"]) +
  geom_bar(aes(x = land_cover_short_class, y = volume_fraction, fill = ratio_std_quantile_brk), stat = "identity") +
...
### Biome ----
biome_stats <- evap_stats_masks[,.(stats_volume = sum(evap_volume), stats_area = sum(area)),.(ratio_std_quantile_brk, biome_short_class)]
biome_stats <- biome_stats[complete.cases(biome_stats)]
biome_stats[, sum_volume:= sum(stats_volume), .(biome_short_class)]
biome_stats[, sum_area:= sum(stats_area), .(biome_short_class)]
biome_stats[, volume_fraction:= stats_volume/sum_volume]
biome_stats[, area_fraction:= stats_area/sum_area]

biome_stats <- biome_stats[complete.cases(biome_stats)]

data_high <- biome_stats[ratio_std_quantile_brk == "dry > wet             ",
                         .(fraction = sum(volume_fraction)), biome_short_class]
data_high[, rank := rank(-fraction)]
data_high <- data_high[order(fraction)]
...
fig_b_spi_biome  <- ggplot(biome_stats) +
  geom_bar(aes(x = biome_short_class, y = volume_fraction, fill = ratio_std_quantile_brk), stat = "identity") +
  xlab('Biome')  +
  ylab('Fraction [-]')  +
  labs(fill = "SPI\nstandardized IQR")  +
  scale_fill_manual(values = c("dry > wet             " = colset_RdBu_5[1], 
...
evap_stats[, ratio_std_quantile_brk := factor(ratio_std_quantile_brk, levels = c("(0,0.9]", "(0.9,1.11]", "(1.11,500]"),
                                              labels = c("wet > dry             ",
                                                         ...
                                                         "dry > wet             "), 
                                              ordered = T),]
evap_stats_masks <- merge(evap_stats, evap_mask, by = c("lon", "lat"))

evap_stats_masks <- evap_mean[evap_stats_masks, on = .(lon, lat)]

### Global ----
global <- evap_stats_masks[,.(stats_volume = sum(evap_volume, na.rm  = T), stats_area = sum(area, na.rm  = T)),.(ratio_std_quantile_brk)]
global[, fraction := stats_volume/sum(stats_volume)]
global[, area_fraction := stats_area/sum(stats_area)]
global
### Land cover ----
...
ggsave(paste0(PATH_SAVE_PARTITION_EVAP_FIGURES, "main/fig4_SQR_aridity_indices_volume.png"), 
       width = 8, height = 12)
