...
### IPCC ----
ipcc_stats <- evap_stats_masks[,.(stats_volume = sum(evap_volume)),.(ratio_std_quantile_brk, IPCC_ref_region)]
ipcc_stats <- ipcc_stats[complete.cases(ipcc_stats)]
ipcc_stats[, ipcc_area:= sum(stats_volume), .(IPCC_ref_region)]
ipcc_stats[, ipcc_fraction:= stats_volume/ipcc_area]
...
ggsave(paste0(PATH_SAVE_PARTITION_EVAP_FIGURES, "supplement/fig4_SI_IPCC_SQR_aridity_indices_volume.png"), 
       width = 14, height = 16)
