...
unique_landclass <- unique(evap_mask$IPCC_ref_region)
...
for (j in 1:length(EVAP_GLOBAL_DATASETS)){
  for (k in 2000:2019){
    ...
    for (i in 1:length(unique_landclass)){
      dummy_mat1 <- evap_mask[evap_mask$IPCC_ref_region %in% unique_landclass[i],]
      ...
      if (j==1 && k==2000 && i==1){
        dummy_mat <- data.frame(year=k,dataset=EVAP_GLOBAL_DATASETS[j],
                                IPCC_ref_region=unique_landclass[i],mean_evp=mean_val)
      }else{
        dummy_mat <- dummy_mat %>% 
...
#saveRDS(dummy_mat, paste0(PATH_SAVE_PARTITION_EVAP, "evap_mean_yearwise_IPCC.rds"))
#####################################################################################
...
dummy_mat_subset <- dummy_mat[dummy_mat$IPCC_ref_region %in% IPCC_region,]

####################################################################################
dummy_agreement <- array(NA, dim=c(n_datasets_2000_2019,n_datasets_2000_2019,
                                   length(unique_landclass)))

for (k in 1:length(IPCC_region)){
  for (i in 1:n_datasets_2000_2019){
    for (j in 1:n_datasets_2000_2019){
      
      dummy_mat_1 <- dummy_mat_subset[dummy_mat_subset$dataset %in% EVAP_GLOBAL_DATASETS[i] &
                                 dummy_mat_subset$IPCC_ref_region %in% IPCC_region[k],]
      
      dummy_mat_2 <- dummy_mat_subset[dummy_mat_subset$dataset %in% EVAP_GLOBAL_DATASETS[j] &
                                 dummy_mat_subset$IPCC_ref_region %in% IPCC_region[k],]
...
ggsave(paste0(PATH_SAVE_PARTITION_EVAP_FIGURES, "heatmap_dist_match_IPCC_Samerica.png"), 
       width = 16, height = 8)
