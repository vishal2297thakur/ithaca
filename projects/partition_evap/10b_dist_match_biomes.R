...
unique_landclass <- unique(evap_mask$biome_class)
unique_landclass <- na.omit(unique_landclass)
unique_landclass <- unique_landclass[-6]
...
for (j in 1:length(EVAP_GLOBAL_DATASETS)){
  for (k in 2000:2019){
    ...
    for (i in 1:length(unique_landclass)){
      dummy_mat1 <- evap_mask[evap_mask$biome_class %in% unique_landclass[i],]
...
#saveRDS(dummy_mat, paste0(PATH_SAVE_PARTITION_EVAP, "evap_mean_yearwise_biomes.rds"))
...
unique_landclass_mod <- as.matrix(c("Tundra","B. Forests","T/S Dry BL Forests","T/S Moist BL Forests",
                            "T/S Coni. Forests","T. Coni. Forests","T. BL Forests",
                            "T. Grasslands","T/S Grasslands","M. Grasslands","Flooded",
                            "Mangroves","Deserts","Mediterranean")) 
...
for (i in 1:1:length(unique_landclass)){
  ...
  plotlist[[i]] <- ggplot(df, aes(x = x, y = y, fill = factor(value))) +
    ...
    coord_fixed()+
    ggtitle(unique_landclass_mod[i])+
...
ggsave(paste0(PATH_SAVE_PARTITION_EVAP_FIGURES, "heatmap_dist_match_biomes.png"), 
       width = 16, height = 8)
