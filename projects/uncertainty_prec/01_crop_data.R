...
source('source/uncertainty_prec.R.R')
...
foreach(data_idx = 1:n_datasets_2000_2019) %dopar% {
  ...
  name_idx <- gsub('/.*', '', PREC_NAMES_2000_2019_FULL_RECORD[data_idx])
  nc_out <- paste0(PATH_SAVE_UNCERTAINTY_PREC_RAW,
                   PREC_NAMES_SHORT_2000_2019_FULL_RECORD[[data_idx]],
...
dummy <- strsplit(PREC_NAMES_2000_2019, split = '//')
dummy <- sapply(dummy, "[[", 2)
dummy <- strsplit(dummy, split = '_')
PREC_NAMES_SHORT_2000_2019 <- sapply(dummy, "[[", 1)

save(PREC_NAMES_2000_2019, PREC_NAMES_SHORT_2000_2019, file = paste0(PATH_SAVE_UNCERTAINTY_PREC, "prec_names_2000_2019.Rdata"))
