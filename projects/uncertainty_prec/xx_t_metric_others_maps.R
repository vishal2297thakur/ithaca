...
to_plot_country$name <- levels(DATASETS)[to_plot_country$name] %>% factor(levels(DATASETS))
...
p01 <- ggplot(to_plot_country) +
  ...
  scale_fill_manual(values = c("gpm-imerg" = "#1f78b4", "mswep" = "#cab2d6",
                               ...
                               "terraclimate" = "#6a3d9a", "gpcc" = "#ffff99",
                               "cpc" = "#b15928"), drop = FALSE,
...
to_plot_ipcc$name <- levels(DATASETS)[to_plot_ipcc$name] %>% factor(levels(DATASETS))
...
prec_month <- merge(prec_masks[basin_name != "", .(lon, lat, basin_name)], prec_month,
                    by = "basin_name")
...
to_plot_basin$name <- levels(DATASETS)[to_plot_basin$name] %>% factor(levels(DATASETS))

p03 <- ggplot(to_plot_basin) +
  geom_sf(data = world_sf, fill = "gray69", color = "gray69") +
  geom_sf(aes(color = name, fill = name)) +
...
to_plot_kg$name <- levels(DATASETS)[to_plot_kg$name] %>% factor(levels(DATASETS))
...
ggsave(plot = p00,
       paste0(PATH_SAVE_UNCERTAINTY_PREC_FIGURES,
              "min_maps.png"), width = 4.5*GOLDEN_RATIO*2, height = 4.5*2)
...
ggsave(plot = p00,
       paste0(PATH_SAVE_UNCERTAINTY_PREC_FIGURES,
              "top10_maps.png"), width = 4.5*GOLDEN_RATIO*2, height = 4.5*2)
...
ggsave(plot = p00,
       paste0(PATH_SAVE_UNCERTAINTY_PREC_FIGURES,
              "ncep_maps.png"), width = 4.5*GOLDEN_RATIO*2, height = 4.5*2)
####
...
ggsave(plot = p00,
       paste0(PATH_SAVE_UNCERTAINTY_PREC_FIGURES,
              "gpm_maps.png"), width = 4.5*GOLDEN_RATIO*2, height = 4.5*2)
