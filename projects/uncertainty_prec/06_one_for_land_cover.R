...
prec_masks <- prec_masks[unique(prec_years[, .(lon, lat)]),
                         .(lon, lat, land_cover_short_class), on = .(lon, lat)]
...
## Save
fwrite(prec_month,
       file = paste0(PATH_SAVE_UNCERTAINTY_PREC_TABLES,"land_cover_month.csv"))

fwrite(prec_years,
       file = paste0(PATH_SAVE_UNCERTAINTY_PREC_TABLES,"land_cover_years.csv"))

saveRDS(MC_month, file = paste0(PATH_SAVE_UNCERTAINTY_PREC,
                                "prec_land_cover_month.rds"))

saveRDS(MC_years, file = paste0(PATH_SAVE_UNCERTAINTY_PREC,
                                "prec_land_cover_years.rds"))
