...
prec_basins <- foreach(dataset_count = 1:n_prec_datasets, .combine = rbind) %dopar% {
  ...
  ingolstadt <- fldmean(crop_data(dummy, paste0(PATH_SAVE_ERA5LAND_BUDGET_SPATIAL, '/era5_basins/Ingolstadt_Europe.shp')))
  ...
  ingolstadt$basin <- 'ingolstadt'
  lugh_ganana$basin <- 'lugh_ganana'
  dummy_2 <- rbind(austin, dhalegaon, ingolstadt, lugh_ganana)
...
ingolstadt_map <- crop_data(dummy, paste0(PATH_SAVE_ERA5LAND_BUDGET_SPATIAL, '/era5_basins/Ingolstadt_Europe.shp'))
