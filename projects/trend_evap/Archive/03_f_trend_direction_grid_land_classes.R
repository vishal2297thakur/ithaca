...
evap_trend_volume <- grid_cell_area[evap_trend_masks, on = .(lon, lat)]
evap_trend_volume[, evap_trend_volume_coeff := area * M2_TO_KM2 * MM_TO_KM] # km3

saveRDS(evap_trend_volume, paste0(PATH_SAVE_EVAP_TREND, "global_grid_evap_area.rds"))

evap_trend_volume[significant_theil_sen*theil_sen_slope > 0, trend_score := 1  ]
evap_trend_volume[significant_theil_sen*theil_sen_slope < 0, trend_score := -1  ]
evap_trend_volume[significant_theil_sen == FALSE, trend_score := 0  ]

evap_trend_volume[trend_score == 1, trend_direction := "positive"]
evap_trend_volume[trend_score == -1, trend_direction := "negative"]
evap_trend_volume[trend_score == 0, trend_direction := "no trend"]
...
ggsave(paste0(PATH_SAVE_EVAP_TREND_FIGURES, "evap_trend_biome_fraction_products.png"), 
       width = 8, height = 12)
...
ggsave(paste0(PATH_SAVE_EVAP_TREND_FIGURES, "evap_trend_ipcc_fraction_products.png"), 
       width = 12, height = 16)
...
ggsave(paste0(PATH_SAVE_EVAP_TREND_FIGURES, "evap_trend_land_cover_fraction_products.png"), 
       width = 8, height = 12)
...
ggsave(paste0(PATH_SAVE_EVAP_TREND_FIGURES, "evap_trend_elevation_fraction_products.png"), 
       width = 8, height = 12)
