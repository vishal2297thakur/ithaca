...
### Figure 1 Magnitude ----
#### Figure 1 prep ----
evap_trend_range <- evap_annual_trend[p <= 0.01 , .(trend_min = min(slope), trend_max = max(slope),p_val = "p < 0.01")]
dummy <- evap_annual_trend[p <= 0.05 , .(trend_min = min(slope), trend_max = max(slope), p_val = "p < 0.05")]

evap_trend_range <- merge(evap_trend_range, dummy, all = T)

dummy <- evap_annual_trend[p <= 0.1 , .(trend_min = min(slope), trend_max = max(slope), p_val = "p < 0.1")]
evap_trend_range <- merge(evap_trend_range, dummy, all = T)
...
ggsave(paste0(PATH_SAVE_EVAP_TREND_FIGURES, "boxplot_DCI_p_val_threshold_bootstrap.png"), 
       width = 8, height = 8)
...
ggsave(paste0(PATH_SAVE_EVAP_TREND_FIGURES, "bar_area_fraction_DCI_p_val_threshold_bootstrap.png"), 
       width = 8, height = 8)

### Figure 2.3 p-value of max |DCI| ----
#### Figure 2.3 prep ----
evap_index[, DCI_max := c(DCI_0_01, DCI_0_05, DCI_0_1, DCI_0_2, DCI_all)[whichDCI_max], .(lat, lon)]

evap_index[, trendDCI_max := c(trend_0_01, trend_0_05, trend_0_1, trend_0_2, trend_all)[whichDCI_max], .(lat, lon)]
...
ggsave(paste0(PATH_SAVE_EVAP_TREND_FIGURES, "boxplot_max_DCI_trend_direction_p_val_threshold_bootstrap.png"), 
       width = 8, height = 8)
...
ggsave(paste0(PATH_SAVE_EVAP_TREND_FIGURES, "bar_area_fraction_max_DCI_by_trend_p_val_threshold_bootstrap.png"), 
       width = 8, height = 8)
...
ggsave(paste0(PATH_SAVE_EVAP_TREND_FIGURES, "map_p_val_opposing_bootstrap.png"), 
       width = 12, height = 8)
...
evap_index[, N_dif_0_05:= abs(N_pos_0_05-N_neg_0_05)]

evap_index[trend_0_05 == "opposing", max(N_dif_0_05)]
hist(evap_index[trend_0_05 == "opposing", N_dif_0_05])

ggplot(evap_index[N_sum_0_05 > 0])+
  geom_tile(aes(x = lon, y = lat, fill = N_sum_0_05))+
  theme_bw()+
  labs(fill = "Significant trends")

evap_index[, N_sum_0_05_brk := cut(N_sum_0_05, breaks = c(1.9,4,6,14))]

hist(evap_index[trend_0_05 == "opposing", N_sum_0_05])
ggplot(evap_index[trend_0_05 == "opposing"])+
  geom_histogram(aes(x = N_dif_0_05, fill = N_sum_0_05_brk), position = "dodge")+
  theme_bw()

evap_index[trend_0_05 == "opposing", N_trend_frac := N_neg_0_05/N_pos_0_05]
