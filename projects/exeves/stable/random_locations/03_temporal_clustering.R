...
names(evap)[3] <- "evap"
exeves[, value := NULL]
exeves_all <- merge(evap, exeves, all.x = TRUE, by = c("KG_class_2", "date"))
...
gg_sample_warm <- ggplot(data = exeves_all[date >= warm_season_start & date <= warm_season_end]) +
  geom_point(data = exeves_all[date >= warm_season_start & date <= warm_season_end & !is.na(event_id)],
             aes(date, 0.6), col = '#a9cce0', size = 2, shape = 15) +
  geom_point(data = exeves_all[date >= warm_season_start & date <= warm_season_end & !is.na(event_qr_id)],
             aes(date, 0.4), col = '#7cb47c', size = 2, shape = 15) +
  geom_point(data = exeves_all[date >= warm_season_start & date <= warm_season_end & !is.na(event_75_id)],
             aes(date, 0.2), col = '#fcc47c', size = 2, shape = 15) +
  geom_point(data = exeves_all[date >= warm_season_start & date <= warm_season_end & !is.na(event_80_id)],
             aes(date, 0), col = '#c07878', size = 2, shape = 15) +
  geom_point(data = exeves_all[date >= warm_season_start & date <= warm_season_end & !is.na(extreme_qr_id)],
  ...
  geom_point(data = exeves_all[date >= warm_season_start & date <= warm_season_end & !is.na(extreme_id)],
...
gg_sample_cold <- ggplot(data = exeves_all[date >= cold_season_start & date <= cold_season_end]) +
  geom_point(data = exeves_all[date >= cold_season_start & date <= cold_season_end & !is.na(event_id)],
             aes(date, -0.05), col = '#a9cce0', size = 2, shape = 15) +
  geom_point(data = exeves_all[date >= cold_season_start & date <= cold_season_end & !is.na(event_qr_id)],
             aes(date, -0.15), col = '#7cb47c', size = 2, shape = 15) +
  geom_point(data = exeves_all[date >= cold_season_start & date <= cold_season_end & !is.na(event_75_id)],
             aes(date, -0.25), col = '#fcc47c', size = 2, shape = 15) +
  geom_point(data = exeves_all[date >= cold_season_start & date <= cold_season_end & !is.na(event_80_id)],
             aes(date, -0.35), col = '#c07878', size = 2, shape = 15) +
  geom_point(data = exeves_all[date >= cold_season_start & date <= cold_season_end & !is.na(extreme_qr_id)],
             aes(date, evap), col = colset_subdued_prof[3], size = 4, shape = 0) +
  geom_line(aes(date, evap), col = colset_subdued_prof[3]) +
  geom_point(data = exeves_all[date >= cold_season_start & date <= cold_season_end & !is.na(event_id)],
             aes(date, evap), col = colset_subdued_prof[2], size = 3, alpha = 0.5) +
  geom_point(data = exeves_all[date >= cold_season_start & date <= cold_season_end & !is.na(extreme_id)],
             aes(date, evap), col = colset_subdued_prof[4]) +
  scale_x_date(expand = c(0, 0), date_breaks = "1 month", minor_breaks = NULL, date_labels = "%b") +
  scale_y_continuous(labels = axis_decimal) + 
  xlab("Time (day)") + 
...
dummy <- exeves[, .(KG_class_2, std_value), period]
exeves_acf <- dummy[, sapply(.SD, function(x) acf(x, lag.max = max_lag, plot = FALSE)$acf), .(KG_class_2, period)]
exeves_acf[, lag := rep(1:(1 + max_lag), n_grids), period]
acf_table_1 <- dcast(exeves_acf[period == 'up_to_2001'], KG_class_2~lag, value.var = 'V1')
acf_lag_means_1 <- apply(acf_table_1, 2, mean)[-1]
acf_table_2 <- dcast(exeves_acf[period == 'after_2001'], KG_class_2~lag, value.var = 'V1')
