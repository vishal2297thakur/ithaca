...
spatial_info_region <- readRDS(paste0(PATH_OUTPUT_DATA, "spatial_info_", region, ".rds"))
borders <- read_sf('../../shared/data/geodata/maps/admin/czechia/CZE_adm0.shp')

axis_decimal <- function(x) sprintf("%.1f", x)

names(prec)[3] <- "prec"
exeves_prec <- merge(spatial_info_region[, .(grid_id, land_cover_class)], exeves_prec, by = "grid_id")
...
exeves_prec_means <- exeves_prec_means[, .(prec, evap, diff_pe, 
                                           diff_prec = diff(prec), 
                                           diff_evap = diff(evap), period), 
                                       .(grid_id, land_cover_class )]
...
gg_all <- ggplot(to_plot) +
  ...
  facet_wrap(~`Land cover`, scales = 'free') + 
...
to_plot_nested <- to_plot[, .N, .(Conditions, `Land cover`)]
to_plot_nested <- to_plot_nested[order(`Land cover`, Conditions)]
...
gg_all <- gg_all + annotation_custom(sub_grob, xmin  = 1, xmax = 1.5, ymin = 1.05, ymax = 1.4)

gg_all_map <- ggplot(to_plot) + 
  geom_tile(
    aes(
      lon, 
      lat, 
