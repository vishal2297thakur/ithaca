...
# All days
exeves_prec_sums <- unique(exeves_prec[month(date) == 6, .(evap = sum(evap) / (SUB_PERIOD_YEARS * 30), 
                                                           prec = sum(prec) / (SUB_PERIOD_YEARS * 30), 
                                                           diff_pe = (sum(prec) - sum(evap)) / (SUB_PERIOD_YEARS * 30)), 
                                       by = .(grid_id, period)])
...
# Exeve days
exeves_prec_sums <- unique(exeves_prec[!is.na(event_80_95_id) & month(date) == 6, .(evap = sum(evap) / (SUB_PERIOD_YEARS * 30), 
                                                                                    prec = sum(prec) / (SUB_PERIOD_YEARS * 30), 
                                                                                    diff_pe = (sum(prec) - sum(evap)) / (SUB_PERIOD_YEARS * 30)), 
                                       by = .(grid_id, period)])
exeves_prec_sums <- exeves_prec_sums[, .(prec, evap, diff_pe, diff_prec = diff(prec), diff_evap = diff(evap), period), .(grid_id)]
