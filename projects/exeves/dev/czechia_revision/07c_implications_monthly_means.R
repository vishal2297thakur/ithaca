...
exeves_prec_means <- exeves_prec_means[, .(prec, evap, diff_pe, diff_prec = diff(prec), diff_evap = diff(evap), period), .(month)]
...
gg_all <- ggplot(to_plot) +
  ...
  geom_text(data = month_names, aes(x, y, label = text), cex = 3.5, nudge_x = 0.1, nudge_y = 0.1, col = 'grey40') + 
...
# Exeve days
exeves_prec_means <- unique(exeves_prec[!is.na(event_80_95_id), .(evap = mean(evap), 
                                                                 prec = mean(prec), 
                                                                 diff_pe = (mean(prec) - mean(evap))), 
...
ggsave(paste0(PATH_OUTPUT_FIGURES, "implications_monthly_means.png"), width = 8, height = 12)
