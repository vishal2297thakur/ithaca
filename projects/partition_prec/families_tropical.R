...
grid_tropical <- prec_masks[KG_class_1_name == "Tropical", .(lon, lat)]
prec_datasets_tropical <- prec_datasets[grid_tropical, on = .(lon, lat)]
prec_datasets_mean <- prec_datasets_tropical[, .(prec = mean(prec, na.rm = TRUE)), .(dataset)]
