...
### Input Data generated in projects/chaning_prec/bootstrap/01_b (seems from 01_c instead)
prec_trend <- readRDS(paste0(PATH_SAVE_CHANGING_PREC, "01_c_global_grid_per_dataset_prec_slope_bootstrap.rds"))  

## Analysis ----
prec_trend[, trend_direction := factor(trend_direction, level = c("positive significant", "positive", "negative", "negative significant"), ordered = T),]
...
### Biome types ----
biome_trends <- prec_trend_masks[,.(trend_area = sum(area)),.(trend_direction, biome_class, dataset)]
...
### ipcc ref regions  ----
ipcc_trends <- prec_trend_masks[,.(trend_area = sum(area)),.(trend_direction, IPCC_ref_region, dataset)]
...
### land use ----
land_trends <- prec_trend_masks[,.(trend_area = sum(area)),.(trend_direction, land_cover_short_class, dataset)]
...
### elevation ----
elev_trends <- prec_trend_masks[,.(trend_area = sum(area)),.(trend_direction, elev_class, dataset)]
...
### KG 3 ----
KG_class_3_trends <- prec_trend_masks[,.(trend_area = sum(area)),.(trend_direction, KG_class_3, dataset)]
...
## Save data ----
saveRDS(biome_trends, paste0(PATH_SAVE_CHANGING_PREC, "02_a_biome_cumulative_trend_direction_per_dataset_bootstrap.rds"))
saveRDS(ipcc_trends, paste0(PATH_SAVE_CHANGING_PREC, "02_a_ipcc_cumulative_trend_direction_per_dataset_bootstrap.rds"))
saveRDS(land_trends, paste0(PATH_SAVE_CHANGING_PREC, "02_a_land_cover_cumulative_trend_direction_per_dataset_bootstrap.rds"))
saveRDS(elev_trends, paste0(PATH_SAVE_CHANGING_PREC, "02_a_elevation_cumulative_trend_direction_per_dataset_bootstrap.rds"))
saveRDS(KG_class_3_trends, paste0(PATH_SAVE_CHANGING_PREC, "02_a_KG_3_cumulative_trend_direction_per_dataset_bootstrap.rds"))
