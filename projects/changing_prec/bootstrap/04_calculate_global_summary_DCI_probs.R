...
#prec_indices <- readRDS(paste0(PATH_SAVE_CHANGING_PREC, "global_grid_slope_indices.rds"))
prec_indices <- readRDS(paste0(PATH_SAVE_CHANGING_PREC, "01_d_global_grid_slope_indices_opp_allowed_bootstrap.rds"))

prec_indices[, DCI_theil_sen_brks := cut(DCI_theil_sen, breaks = c(1, 0.35, 0.01, -0.01, -0.35, -1))]

world <- prec_indices[, sum(area)]
trend_fraction <- prec_indices[, (sum(area)/world), trend]
trend_fraction[, sum(V1)]

DCI_fraction <- prec_indices[, sum(area)/world, DCI_theil_sen_brks]
DCI_fraction[, sum(V1)]
...
ggsave(paste0(PATH_SAVE_CHANGING_PREC_FIGURES, "04_pie_global_probability_groups.png"), 
       width = 6, height = 6)
...
ggsave(paste0(PATH_SAVE_CHANGING_PREC_FIGURES, "04_pie_global_DCI_theil_sen.png"), 
       width = 6, height = 6)
