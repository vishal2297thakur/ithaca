...
# Maps ---
prec_index[p_val_opposing == "1", p_val_opposing := "no opposing\ntrends"]
prec_index[p_val_opposing == ">0.2", p_val_opposing := "p <= 1"]
prec_index[p_val_opposing == "<=0.01", p_val_opposing := "p < 0.01"]
prec_index[p_val_opposing == "<=0.05", p_val_opposing := "p < 0.05"]
prec_index[p_val_opposing == "<=0.1", p_val_opposing := "p < 0.1"]
prec_index[p_val_opposing == "<=0.2", p_val_opposing := "p < 0.2"]

prec_index[, p_val_opposing := as.factor(p_val_opposing)]

prec_index[, p_val_opposing := factor(p_val_opposing, levels = 
                                        c("p < 0.01", "p < 0.05", "p < 0.1", "p < 0.2", "p <= 1", "no opposing\ntrends"))]

prec_index[N_none_0_2 >= 7, more_sig_trends := "p <= 1" ]
prec_index[N_none_0_2 < 7, more_sig_trends := "p < 0.2" ]
prec_index[N_none_0_1 < 7, more_sig_trends := "p < 0.1" ]
prec_index[N_none_0_05 < 7, more_sig_trends := "p < 0.05" ]
prec_index[N_none_0_01 < 7, more_sig_trends := "p < 0.01" ]
prec_index[, more_sig_trends := as.factor(more_sig_trends) ]
prec_index[, more_sig_trends := factor(more_sig_trends, levels = 
                                         c("p < 0.01", "p < 0.05", "p < 0.1", "p < 0.2", "p <= 1"))]

prec_index[, DCI_all_brk := cut(DCI_all, c(-1.01,-0.5,-0.07, 0.07,0.5,1))]
prec_index[DCI_all_brk == "(-1.01,-0.5]", DCI_all_brk := "[-1,-0.5]"]
prec_index[, DCI_all_brk:= factor(DCI_all_brk, levels = c("[-1,-0.5]", 
                                                          "(-0.5,-0.07]", "(-0.07,0.07]", "(0.07,0.5]",
                                                          "(0.5,1]"))]


## Save subset of grid data ----
prec_index_sel <- subset(prec_index, select = c("lon", "lat", "DCI_all_brk", "more_sig_trends", "p_val_opposing"))
saveRDS(prec_index_sel, paste0(PATH_SAVE_CHANGING_PREC_TABLES, "data_fig_2_a_c_d_grid_trend_stats.rds"))


# Area fractions ----
grid_cell_area <- unique(prec_index[, .(lon, lat)]) %>% grid_area() # m2
...
saveRDS(data_dci_area, paste0(PATH_SAVE_CHANGING_PREC_TABLES, "data_fig_2_f_area_stats_DCI_all_trends.rds"))
...
saveRDS(data_sel_trend_area, paste0(PATH_SAVE_CHANGING_PREC_TABLES, "data_fig_2_d_area_stats_trend_direction.rds"))
